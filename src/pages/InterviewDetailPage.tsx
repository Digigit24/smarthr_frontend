    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium', typeCfg.bg, typeCfg.color)}>
                  {typeCfg.label}
                </span>
              </div>
              <h1 className="text-xl sm:text-2xl font-bold">{typeCfg.label} Interview</h1>
              <p className="text-sm text-muted-foreground mt-0.5">
                {formatDateTime(interview.scheduled_at)} · {interview.duration_minutes} min
              </p>
              <p className="text-xs text-muted-foreground mt-1">Created {formatDateTime(interview.created_at)} · Updated {formatDateTime(interview.updated_at)}</p>
            </div>
            {/* Actions */}
            <div className="flex flex-wrap gap-2 shrink-0">
              ...
              <Button size="sm" variant="outline" onClick={() => navigate(`/interviews/${id}/edit`)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 space-y-4 sm:space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/30">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{formatDateTime(interview.scheduled_at)}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/30">
          ...
          {(interview.feedback || interview.rating) && (
            <div className="rounded-xl border bg-card p-4 sm:p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-semibold text-sm">Feedback & Rating</h3>
              </div>
              {interview.rating != null && (
                <div className="mb-3 flex items-center gap-2">
                  <div className="flex gap-0.5">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} className={cn('h-5 w-5', i < interview.rating! ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground/20')} />
                    ))}
                  </div>
                  <span className="text-sm font-semibold text-amber-600">{interview.rating}/5</span>
                </div>
              )}
              {interview.feedback && (
          ...
          {completeOpen && (
            <div className="rounded-xl border-2 border-emerald-200 dark:border-emerald-800/50 bg-card p-4 sm:p-5">
              <div className="flex items-center gap-2 mb-4">
                <div className="h-7 w-7 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center">
                  <CheckCircle className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                </div>
                <h3 className="font-semibold text-sm">Complete Interview</h3>
              </div>
              <form
              ...
              >
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" min="1" max="5" placeholder="e.g. 4" {...completeRegister('rating')} className="w-24" />
                </div>
                <div className="flex gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => setCompleteOpen(false)}>
                    Cancel
                  </Button>
        ...
        <div className="space-y-4">
          ...
          <div className={cn('rounded-xl border-2 p-4 sm:p-5',
            ...
            <span className={cn('inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold', statusCfg.color)}>
          ...
          <div className="rounded-xl border bg-card p-4 sm:p-5 space-y-3">
            ...
            <div className="space-y-2.5 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> Type</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" /> Calendar</span>
          ...
          {interview.meeting_link && isUpcoming && (
            <a href={interview.meeting_link} target="_blank" rel="noopener noreferrer"
              className="block rounded-xl border bg-card p-4 hover:border-primary/50 transition-colors group">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-lg bg-green-50 dark:bg-green-900/20 flex items-center justify-center">
                  <Video className="h-5 w-5 text-green-600 dark:text-green-400" />
                </div>
                <div>
                  <p className="font-medium text-sm group-hover:text-primary transition-colors">Join Meeting</p>
                  <p className="text-xs text-muted-foreground">Open video call</p>
                </div>
          ...
          <div className="rounded-xl border bg-card p-4 sm:p-5 space-y-2">
            ...
            <Button variant="outline" className="w-full justify-start" size="sm" onClick={() => navigate(`/interviews/${id}/edit`)}>
              <Pencil className="h-4 w-4 mr-2" /> Edit Interview
            </Button>
            {interview.status !== 'CANCELLED' && interview.status !== 'COMPLETED' && (
              <Button variant="outline" className="w-full justify-start" size="sm" onClick={() => cancelMutation.mutate()}>
            ...
            <Button variant="ghost" className="w-full justify-start text-destructive hover:text-destructive" size="sm"
              onClick={() => { if (window.confirm('Delete this interview?')) deleteMutation.mutate() }}
            >
              <Trash2 className="h-4 w-4 mr-2" /> Delete Interview
            </Button>
          </div>
        </div>
      </div>
